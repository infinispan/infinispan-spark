...
docker exec -it $SPARK_MASTER_NAME /usr/local/spark/bin/spark-shell --master spark://$SPARK_MASTER:7077 --jars /usr/local/code/scala-2.11/infinispan-spark-twitter.jar --conf "spark.infinispan=$INFINISPAN_IP"
